...
router.post("/todo-lists/:listId/collaborators", async (req, res) => {
  try {
    ...
    if (todoList.collaborators.some((c) => c.userId.equals(collaborator._id))) {
    ...
    res
      .status(201)
      .json({ data: collaborator, message: "Collaborator added successfully" });
  } catch (error) {
